...
const navigation = [
  { key: 'home', name: 'בית', href: '/', current: true,requiredLogin:false },
  { key: 'coupons', name: 'קופונים שלי', href: '/coupons', current: false,requiredLogin:true },
  { key: 'account', name: 'החשבון שלי', href: '/account', current: false,requiredLogin:true },
...
const userNavigation = [
  { key: 'login', name: 'התחברות', href: '/login',requiredLogin:false },
  { key: 'register', name: 'הרשמה', href: '/register',requiredLogin:false },
  { key: 'logout', name: 'התנתקות', href: '/logout',requiredLogin:true },
]
export const NavBar = () => {
  ...
  return (
    <nav className="bg-gray-600 fixed top-0 w-full">
                  ...
                  <div className="mr-4 flex items-baseline space-x-4">
                    {navigation.map((item) => (
                      <Link
                        key={item.name}
                        href={item.href}
                        className={`${item.requiredLogin ? 'hidden' : 'block'} rounded-md px-3 text-center py-2 text-sm font-medium ${item.current ? 'bg-gray-700 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white'}`}
        ...
        <div className={`md:hidden ${mobileMenuOpen?'block':'hidden'}`} id="mobile-menu">
          <div className="space-y-1 px-2 pt-2 pb-3 sm:px-3">
            {navigation.map((item) => (
              <Link key={item.name} href={item.href} className="block rounded-md px-3 py-2 text-base font-medium text-gray-300 hover:bg-gray-700 hover:text-white" aria-current={item.current ? 'page' : undefined}>{item.name}</Link>
            ))}
          </div>
          <div className="border-t border-gray-700 pt-4 pb-3">
            <div className="space-y-1 px-2">
            {userNavigation.map((item) => (
              <Link key={item.name} href={item.href} className="block rounded-md px-3 py-2 text-base font-medium text-gray-400 hover:bg-gray-700 hover:text-white" aria-current={item.current ? 'page' : undefined}>{item.name}</Link>
            ))}
            </div>
          </div>
        </div>
